...
import (
	...
	"github.com/balamurugana/goat/csv"
	"github.com/balamurugana/goat/json"
	"github.com/balamurugana/goat/parquet"
...
func (s3Select *S3Select) Open(getReader func(offset, length int64) (io.ReadCloser, error)) error {
	...
	case csvFormat:
		...
		s3Select.recordReader, err = csv.NewReader(s3Select.progressReader, &s3Select.Input.CSVArgs, s3Select.statement.TableAlias())
		if err != nil {
			return err
		}
	...
	case jsonFormat:
		...
		s3Select.recordReader = json.NewReader(s3Select.progressReader, &s3Select.Input.JSONArgs, s3Select.statement.TableAlias())
		return nil
	case parquetFormat:
		var err error
		s3Select.recordReader, err = parquet.NewReader(getReader, &s3Select.Input.ParquetArgs, s3Select.statement.TableAlias())
		return err
	}
...
// Send - starts sending filtered records as per selection query to http response writer.
func (s3Select *S3Select) Send(w http.ResponseWriter) {
	...
	sendRecord := func() bool {
		...
		if data, err = s3Select.marshal(record); err != nil {
			return false
		}

		if err = writer.SendRecords(data); err != nil {
			fmt.Println("send records failed.", err)
			err = nil
			return false
		}
	...
	for {
		if record, err = s3Select.recordReader.Read(); err != nil {
			if err != io.EOF {
				break
			}

			if record, err = s3Select.statement.AggregateResult(); err != nil {
				break
			}
			...
			if err = writer.SendStats(s3Select.getProgress()); err != nil {
				fmt.Println("send stats failed.", err)
				err = nil
			}
		...
		if record, err = s3Select.statement.Eval(record); err != nil {
			break
		}
	...
	if err != nil {
		if serr := writer.SendError("InternalError", err.Error()); serr != nil {
			fmt.Println("unable to send error message", err, "error:", serr)
		}
	}
}
...
func NewS3Select(r io.Reader) (*S3Select, error) {
	var s3Select S3Select
	err := xml.NewDecoder(r).Decode(&s3Select)
	return &s3Select, err
}
