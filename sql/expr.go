...
// Call - evaluates underlaying expression for given record and returns evaluated result.
...
// Call - returns given args as map value.
...
func (expr *columnExpr) Eval(record *Record) (*Value, error) {
	value, ok := record.Get(expr.name)
	if !ok {
		err := fmt.Errorf("record does not have value for %v", expr.name)
...
func newColumnExpr(colName *sqlparser.ColName) *columnExpr {
