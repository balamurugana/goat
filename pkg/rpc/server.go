...
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err := gob.NewDecoder(newGobReader(req.Body)).Decode(&callReq); err != nil {
	...
	replyBody, err = server.call(callReq.Method, callReq.ArgBytes, req.Body, replyBuf)
	if err != nil {
		callRes.Error = err.Error()
	}
	callRes.ReplyBytes = replyBuf.Bytes()

	responseBuf := bufPool.Get()
	if err = gob.NewEncoder(responseBuf).Encode(callRes); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		// FIXME: add body with what went wrong
