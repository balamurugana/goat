...
func (r *Reader) Read() (*sql.Record, error) {
	...
	for name, v := range parquetRecord {
		...
		case parquetgen.Type_BOOLEAN:
			record.Set(name, sql.NewBool(v.Value.(bool)))
		case parquetgen.Type_INT32:
			record.Set(name, sql.NewInt(int64(v.Value.(int32))))
		case parquetgen.Type_INT64:
			record.Set(name, sql.NewInt(v.Value.(int64)))
		case parquetgen.Type_FLOAT:
			record.Set(name, sql.NewFloat(float64(v.Value.(float32))))
		case parquetgen.Type_DOUBLE:
			record.Set(name, sql.NewFloat(v.Value.(float64)))
		case parquetgen.Type_INT96, parquetgen.Type_BYTE_ARRAY, parquetgen.Type_FIXED_LEN_BYTE_ARRAY:
			record.Set(name, sql.NewString(string(v.Value.([]byte))))
		}
	}
...
func NewReader(getReaderFunc func(offset, length int64) (io.ReadCloser, error), args *ReaderArgs, alias string) (*Reader, error) {
